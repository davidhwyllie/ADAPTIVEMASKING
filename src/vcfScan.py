...
class regionScan_from_genbank(vcfScan):
	...
	The regions can be defined in two ways:
		* genes - annotate genes and intergenic regions
	...
	# read the genbank file; define regions based on genes	
	genbank_file_name = os.path.join("..", "testdata", "NC_0103971.1.gb")
	rs = regionScan_from_genbank(genbank_file_name, method = 'gene')

	# alternative where the genbank file is not reparsed,
	# but rather the .regions property of a previous object
	# is used to create a new one	
	genbank_file_name = os.path.join("..", "testdata", "NC_0103971.1.gb")
	rs = regionScan_from_genbank(genbank_file_name, method = 'gene')
	region1 = rs.regions
	rs2 = regionScan_from_genbank(region1)
	...
	def __init__(self,
		...
		Arguments:
			genbank_file_name:  either: a pandas data frame generated by regionScan_from_genbank.regions OR
								or		NCBI genbank format file used to define the regions of interest [str]
			method: either 'gene' or 'All'.  Ignored if regenerating object based on stored regions.
			* method = 'gene' - annotate genes and intergenic regions.
							 For example, if there are genes G1, G2, and G3
							 with boundaries 100..1000, 1200..2000, and 2200..3000,
							 ...
							 2001..2199 'near_G3'
							 2200..3000 'G3'
			* method = 'All' - annotate just one region, covering the entire genome.  Ignored if regenerating object based on stored regions.
		    ...
		    * min_region_size: the minimum size of a region.  Ignored if regenerating object as above.
		...
		feature_types=set()
		# check whether the genbank_file_name passed is a dataframe or a file name;
		if isinstance(genbank_file_name, pd.core.frame.DataFrame):
			observed = set(genbank_file_name.columns.values.tolist())
			expected = set(['end_pos','start_pos','roi_name','length','mid'])
			if observed == expected:
				# we are passed a valid set of precomputed regions
				self.regions = genbank_file_name
			else:
				raise ValueError("Was passed a pandas data frame(?with regions) but the column names are not as expected.  Observed: {0} Expected {1}".format(observed, expected))
			...
			if method == 'All':
				...
				# extract the gene boundaries and any intergenic regions.
				previous_end = -1
				feature_id = -1
				feature_list = []
				...
				for feature in record.features:
					...
					if feature.type == 'gene':
						...
						if not current_start == previous_end +1:
							feature_id +=1
							feature_list.append({'id':feature_id, 'roi_name':'near_'+cds_name,'start_pos':previous_end+1+1, 'end_pos':current_start-1+1})
							
						# add the current feature	
						feature_id +=1	
						feature_list.append({'id':feature_id, 'roi_name':cds_name,'start_pos':current_start+1, 'end_pos':current_end+1})
			
						previous_end = current_end
					
				if not previous_end == len(record.seq):		# there's an extra bit at the right hand end
					feature_id +=1
					feature_list.append({'id':feature_id, 'roi_name':'near_end', 'start_pos':previous_end+1+1,  'end_pos':len(record.seq)})
				...
				for id in features_merged.keys():
					...
					while merge_to>0:
						...
						if merge_to in features_final.keys():
							...
							if not features_final[merge_to]['roi_name'][0:4] == 'incl':
								features_final[merge_to]['roi_name'] = 'incl_' + features_final[merge_to]['roi_name']
							break
		
				# create a list of features, and generate a pandas data frame
				feature_list = []
				...
				raise ValueError("Only methods 'gene' and 'All' are supported; got {0}".format(method))
			
		# add ROIs to object from self.regions
		for i in self.regions.index:		
...
class test_regionScan_1a(unittest.TestCase):
	def runTest(self):
		...
		if not os.path.exists(genbank_file_name):
			self.fail("Input file does not exist.  Please see README.  You may need to install test data.")
		rs = regionScan_from_genbank(genbank_file_name, method= 'gene', min_region_size = 0)

		self.assertTrue(isinstance(rs.regions, pd.DataFrame))
		self.assertEqual(len(rs.regions.index), 9895)
...
class test_regionScan_1b(unittest.TestCase):
	def runTest(self):
		...
		self.assertTrue(isinstance(rs.regions, pd.DataFrame))
		self.assertTrue(len(rs.regions.index)< 9895)
...
class test_regionScan_1c(unittest.TestCase):
	...
	def runTest(self):
		...
		self.assertTrue(isinstance(rs.regions, pd.DataFrame))
		self.assertEqual(len(rs.regions.index), 6712)

		# check nothing is missing from our regions, and nothing is covered twice
		# actually, overlapping orfs exist, so this in not a good test.
		bases_covered = set()
		for i in rs.regions.index:
			nt = range(rs.regions.loc[i,'start_pos'], rs.regions.loc[i,'end_pos']+1)
			for pos in nt:
...
class test_regionScan_1d(unittest.TestCase):
	""" test creation of a new regionScan object using the .regions property of a previous one """
	def runTest(self):
		...
		# is used to create a new one	
		genbank_file_name = os.path.join("..", "testdata", "NC_0103971.1.gb")
		rs = regionScan_from_genbank(genbank_file_name = genbank_file_name, method = 'gene')
		region1 = rs.regions
		rs2 = regionScan_from_genbank(genbank_file_name = region1)
		regions2 = rs2.regions
		self.assertEqual(rs2.roi2psn, rs.roi2psn)
